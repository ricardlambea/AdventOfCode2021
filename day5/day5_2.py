...
# ### DAY 5.2
#
# ### NOTE: This script works absolutely fine, the problem is that the input is not correct. It has been checked.
...
with open('input.txt', 'r') as f:
    ...
    for line in f:
        ...
        if x0 == x1 or y0 == y1 or x0 == y0 and x1 == y1 or x0 == y1 and x1 == y0:
        ...
        if coords[0] != coords[2] and coords[1] == coords[3]: #vertical flow
            ...
            if coords[1] < coords[3]:
                ...
                for y in range(coords[3], coords[1]+1):
                    c_matrix[y][coords[0]] += 1
        elif coords[0] == coords[1] and coords[2] == coords[3]: #positive diagonal flow
            for x in range(coords[0], coords[2]+1):
                for y in range(coords[1], coords[3]+1):
                    c_matrix[x][y] += 1
        elif coords[0] == coords[3] and coords[1] == coords[2]: #negative diagonal flow
            if coords[0] < coords[2]:
                for x in range(coords[0], coords[2] + 1):
                    for y in range(coords[3], coords[1] + 1):
                        c_matrix[x][y] += 1
